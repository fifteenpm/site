...
export default class Scene extends Component {
    ...
    updateCameraView() {
        ...
        }, 60 / CONSTANTS.trackTimes[trackName].bpm * 10000);
    }
    ...
    shouldPivotStatue() {
        const { trackName, audioPlayer } = this.props;
        const statuePivotTimes = CONSTANTS.trackTimes[trackName].pivotStatue;
        statuePivotTimes.forEach((t) => {
            if (this.state.inMirrorLand && Math.abs(audioPlayer.currentTime - t) < .1) {
    ...
    shouldEnterWormhole() {
        const { trackName, audioPlayer } = this.props;
        const enterWormholeTimes = CONSTANTS.trackTimes[trackName].enterWormhole;
    ...
    renderScene() {
        const { audioPlayer, trackName, hasEnteredWorld } = this.props;
        if (hasEnteredWorld) {
